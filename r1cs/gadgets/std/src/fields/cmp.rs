use crate::{boolean::Boolean, fields::fp::FpGadget, prelude::*, ToBitsGadget};
...
impl<F: PrimeField> FpGadget<F> {
    ...
    fn is_smaller_than_unchecked<CS: ConstraintSystemAbstract<F>>(
        &self,
        mut cs: CS,
        other: &FpGadget<F>,
    ) -> Result<Boolean, SynthesisError> {
    ...
    /// and `other` are `<= (p-1)/2`.
    fn enforce_smaller_than<CS: ConstraintSystemAbstract<F>>(
        &self,
        mut cs: CS,
        other: &FpGadget<F>,
    ) -> Result<(), SynthesisError> {
    ...
    /// verify that.
    fn enforce_smaller_than_unchecked<CS: ConstraintSystemAbstract<F>>(
        &self,
        mut cs: CS,
        other: &FpGadget<F>,
    ) -> Result<(), SynthesisError> {
...
mod test {
    ...
    use crate::{
        algebra::{fields::tweedle::Fr, PrimeField, UniformRand},
        fields::fp::FpGadget,
    };
    use r1cs_core::{
    ...
    fn test_cmp() {
        ...
        for i in 0..10 {
            ...
            match a.cmp(&b) {
                Ordering::Less => {
                    a_var
                        .enforce_cmp(cs.ns(|| "enforce less"), &b_var, Ordering::Less, false)
                        .unwrap();
                    a_var
                        .enforce_cmp(cs.ns(|| "enforce less equal"), &b_var, Ordering::Less, true)
                        .unwrap();
                }
                Ordering::Greater => {
                    a_var
                        .enforce_cmp(
                            cs.ns(|| "enforce greater"),
                            &b_var,
                            Ordering::Greater,
                            false,
                        )
        ...
        for _i in 0..10 {
            ...
            let a_var = FpGadget::<Fr>::alloc(&mut cs.ns(|| "generate_a"), || Ok(a)).unwrap();
            a_var
                .enforce_cmp(cs.ns(|| "enforce less"), &a_var, Ordering::Less, false)
                .unwrap();
        ...
        for _i in 0..10 {
            ...
            let a_var = FpGadget::<Fr>::alloc(&mut cs.ns(|| "generate_a"), || Ok(a)).unwrap();
            a_var
                .enforce_cmp(cs.ns(|| "enforce less"), &a_var, Ordering::Less, true)
                .unwrap();
            if !cs.is_satisfied() {
                println!("{:?}", cs.which_is_unsatisfied());
            }
