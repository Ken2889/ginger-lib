...
impl Boolean {
        ...
        CS: ConstraintSystemAbstract<ConstraintF>,
    {
        ...
        let run = Self::enforce_smaller_or_equal_than_le(
...
mod test {
    ...
    fn test_smaller_than_or_equal_to() {
        ...
        for i in 0..1000 {
            ...
            let bits = Vec::alloc(&mut cs.ns(|| format!("alloc bits {}", i)), || {
                Ok(native_bits)
            })
            .unwrap();
            Boolean::enforce_smaller_or_equal_than_le(
                cs.ns(|| format!("enforce_smaller_or_equal_than_le {}", i)),
                &bits,
                s.into_repr(),
            )
            .unwrap();
        ...
        for i in 0..1000 {
            ...
            .unwrap();
            Boolean::enforce_smaller_or_equal_than_le(
                cs.ns(|| format!("enforce_smaller_or_equal_than_le s {}", i)),
                &bits,
                s.into_repr(),
            )
            .unwrap();
            ...
            if r < s2 {
                Boolean::enforce_smaller_or_equal_than_le(
                    cs.ns(|| format!("enforce_smaller_or_equal_than_le s2 {}", i)),
                    &bits,
                    s2.into_repr(),
                )
                .unwrap();
