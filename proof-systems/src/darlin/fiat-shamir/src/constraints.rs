...
use r1cs_std::{
    boolean::Boolean, fields::fp::FpGadget, to_field_gadget_vec::ToConstraintFieldGadget,
};

/// the trait for Fiat-Shamir RNG Gadget
pub trait FiatShamirRngGadget<ConstraintF: PrimeField>: Sized + Clone {
    ...
    fn enforce_record<CS, RG>(&mut self, cs: CS, data: RG) -> Result<(), SynthesisError>
    where
    ...
    fn enforce_get_many_challenges<CS: ConstraintSystemAbstract<ConstraintF>, const N: usize>(
        ...
        (0..num)
            .map(|i| self.enforce_get_challenge(cs.ns(|| format!("get challenge {}", i))))
            .collect()
    }
...
pub(crate) mod test {
    ...
    >(
        ...
        let to_record = (0..10)
            ...
            .collect::<Vec<_>>();
        fs_rng
            .enforce_record(cs.ns(|| "record native"), to_record)
            .unwrap();
        ...
        let to_record = (0..10)
            ...
            .collect::<Vec<_>>();
        fs_rng
            .enforce_record(cs.ns(|| "record non native"), to_record.as_slice())
            .unwrap();
        ...
        let to_record = (0..100)
            ...
            .collect::<Vec<UInt8>>();
        fs_rng
            .enforce_record(cs.ns(|| "record bytes"), to_record.as_slice())
            .unwrap();
        ...
        let insert_chal_into_set =
            |chal: [Boolean; N], set: &mut std::collections::HashSet<Vec<bool>>| -> bool {
                set.insert(
                    chal.iter()
                        .map(|bit_g| bit_g.get_value().unwrap())
                        .collect::<Vec<bool>>(),
        ...
        insert_chal_into_set(
            fs_rng
                .enforce_get_challenge::<_, N>(cs.ns(|| "test 2 - get init challenge"))
                .unwrap(),
            &mut set,
        );
        ...
        for i in 1..=10 {
            ...
            outs.iter().for_each(|out_bits| {
                assert!(!out_bits.iter().all(|bit_g| !bit_g.get_value().unwrap()));
            });

            // Bits shouldn't be all 1 with overwhelming probability
            outs.iter().for_each(|out_bits| {
                assert!(!out_bits.iter().all(|&bit| bit.get_value().unwrap()));
            });
