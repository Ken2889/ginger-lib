...
mod marlin {
    ...
    use digest::Digest;
    use poly_commit::{
        ipa_pc::InnerProductArgPC, DomainExtendedPolynomialCommitment, PolynomialCommitment, PCKey,
    };
    use rand::thread_rng;
    ...
    fn test_circuit<G: Group, PC: PolynomialCommitment<G>, D: Digest>(
        ...
        for _ in 0..num_samples {
            ...
            let (index_pk, index_vk) = Marlin::<G, PC>::circuit_specific_setup::<_, D>(&pc_pk, circ).unwrap();
            ...
            let (original_pc_pk, original_pc_vk) = PC::setup::<D>(2 * (num_constraints - 1)).unwrap();
