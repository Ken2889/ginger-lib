...
pub struct ProverState<'a, F: PrimeField> {
    ...
    // the public input polynomial x(X).
    x_poly: Option<DensePolynomial<F>>,
...
impl<F: PrimeField> IOP<F> {
    ...
    pub fn prover_second_round<'a, R: RngCore>(
        ...
        let x_poly = match state.x_poly {
            Some(ref v) => v,
            None => return Err(Error::Other("x_poly is empty".to_owned())),
        };
