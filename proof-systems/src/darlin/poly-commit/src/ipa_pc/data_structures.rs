...
impl<G: Curve> PCKey for CommitterKey<G> {
    ...
    ) -> Result<Self, Error>
    {
        ...
        let trim_time =
            start_timer!(|| format!("Trimming to supported degree of {}", degree));
...
impl<G: Curve> BDFGMultiPointProof<G> for MultiPointProof<G> {
    type Commitment = G;
    type Proof = Proof<G>;
...
impl<G: Curve> CanonicalDeserialize for DLogItem<G> {
    fn deserialize<R: Read>(mut reader: R) -> Result<Self, SerializationError> {
        ...
        Ok(Self { final_comm_key, check_poly })
    }
...
impl<G: Curve> SemanticallyValid for DLogItem<G> {
    fn is_valid(&self) -> bool {
        self.final_comm_key.is_valid() && self.check_poly.0.is_valid()
    }
}
...
impl<G: Curve> Default for DLogItem<G> {
    fn default() -> Self {
        Self {
            final_comm_key: G::default(),
            check_poly: SuccinctCheckPolynomial(vec![]),
        }
    }
}
...
impl<G: Curve> PCVerifierState for DLogItem<G> {}
