...
use crate::{
    DomainExtendedPolynomialCommitment, PolynomialCommitment, PCKey,
};
...
type PC_DEE<'a> = PC<DeeJacobian, Blake2s>;
// its domain extended variant
type PC_DEE_DE<'a> = DomainExtendedPolynomialCommitment<DeeJacobian, PC_DEE<'a>>;
...
fn constant_poly_test() {
    use crate::tests::*;
    constant_poly_test::<_, PC_DEE>(None, true).expect("test failed for tweedle_dee-blake2s");
}

#[test]
fn constant_poly_negative_test() {
    use crate::tests::*;
    match constant_poly_test::<_, PC_DEE>(Some(NegativeType::Values), true) {
    ...
    match constant_poly_test::<_, PC_DEE>(Some(NegativeType::Commitments), true) {
        Err(Error::FailedSuccinctCheck) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
    ...
    match constant_poly_test::<_, PC_DEE>(Some(NegativeType::CommitterKey), true) {
    ...
    match constant_poly_test::<_, PC_DEE>(Some(NegativeType::VerifierKey), true) {
        Err(Error::IncorrectProof) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
...
fn single_poly_test() {
    use crate::tests::*;
    single_poly_test::<_, PC_DEE>(None, true).expect("test failed for tweedle_dee-blake2s");
}

#[test]
fn single_poly_negative_test() {
    use crate::tests::*;
    match single_poly_test::<_, PC_DEE>(Some(NegativeType::Values), true) {
        Err(Error::FailedSuccinctCheck) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
    ...
    match single_poly_test::<_, PC_DEE>(Some(NegativeType::Commitments), true) {
        Err(Error::FailedSuccinctCheck) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
    ...
    match single_poly_test::<_, PC_DEE>(Some(NegativeType::CommitterKey), true) {
        Err(Error::IncorrectProof) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
    ...
    match single_poly_test::<_, PC_DEE>(Some(NegativeType::VerifierKey), true) {
        Err(Error::IncorrectProof) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
...
fn two_poly_four_points_test() {
    use crate::tests::*;
    two_poly_four_points_test::<_, PC_DEE>(None, true).expect("test failed for tweedle_dee-blake2s");
}

#[test]
fn two_poly_four_points_negative_test() {
    use crate::tests::*;
    match two_poly_four_points_test::<_, PC_DEE>(Some(NegativeType::Values), true) {
        Err(Error::FailedSuccinctCheck) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
    ...
    match two_poly_four_points_test::<_, PC_DEE>(Some(NegativeType::Commitments), true) {
        Err(Error::FailedSuccinctCheck) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
    ...
    match two_poly_four_points_test::<_, PC_DEE>(Some(NegativeType::CommitterKey), true) {
        Err(Error::IncorrectProof) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
    ...
    match two_poly_four_points_test::<_, PC_DEE>(Some(NegativeType::VerifierKey), true) {
        Err(Error::IncorrectProof) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
...
fn full_end_to_end_test() {
    use crate::tests::*;
    full_end_to_end_test::<_, PC_DEE>(None, true).expect("test failed for tweedle_dee-blake2s");
...
fn full_end_to_end_negative_test() {
    use crate::tests::*;
    match full_end_to_end_test::<_, PC_DEE>(Some(NegativeType::Values), true) {
        Err(Error::FailedSuccinctCheck) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
    ...
    match full_end_to_end_test::<_, PC_DEE>(Some(NegativeType::Commitments), true) {
        Err(Error::FailedSuccinctCheck) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
    ...
    match full_end_to_end_test::<_, PC_DEE>(Some(NegativeType::CommitterKey), true) {
        Err(Error::IncorrectProof) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
    ...
    match full_end_to_end_test::<_, PC_DEE>(Some(NegativeType::VerifierKey), true) {
        Err(Error::IncorrectProof) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
...
fn key_hash_test() {
    ...
    let (ck, _) = PC_DEE::setup(max_degree).unwrap();
    let trimmed_ck = ck.trim(supported_degree).unwrap();

    assert!(PC_DEE::check_key(&trimmed_ck, max_degree));
    assert!(!PC_DEE::check_key(&trimmed_ck, supported_degree));
    assert!(trimmed_ck.get_hash() == ck.get_hash());

    let h = Blake2s::digest(&to_bytes![&trimmed_ck.comm_key, &trimmed_ck.h, &trimmed_ck.s].unwrap())
        .to_vec();
...
fn segmented_test() {
    use crate::tests::*;
    segmented_test::<_, PC_DEE_DE>(None, true).expect("test failed for tweedle_dee-blake2s");
    println!("Finished tweedle_dee-blake2s");
}
...
fn segmented_negative_test() {
    use crate::tests::*;
    match segmented_test::<_, PC_DEE_DE>(Some(NegativeType::Values), true) {
        Err(Error::FailedSuccinctCheck) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
    ...
    match segmented_test::<_, PC_DEE_DE>(Some(NegativeType::Commitments), true) {
        Err(Error::FailedSuccinctCheck) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
    ...
    match segmented_test::<_, PC_DEE_DE>(Some(NegativeType::CommitterKey), true) {
        Err(Error::IncorrectProof) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
    ...
    match segmented_test::<_, PC_DEE_DE>(Some(NegativeType::VerifierKey), true) {
        Err(Error::IncorrectProof) => {}
        Ok(_) => {
            panic!("test should fail for tweedle_dee-blake2s")
        }
