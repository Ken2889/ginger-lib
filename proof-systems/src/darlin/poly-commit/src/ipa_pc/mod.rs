...
use algebra::{
    to_bytes, BitIterator, Curve, Field, PrimeField, SemanticallyValid, ToBytes, UniformRand,
};
...
impl<G: Curve, D: Digest> InnerProductArgPC<G, D> {
    ...
    pub fn check_key(ck: &CommitterKey<G>, max_degree: usize) -> bool {
        let (original_ck, _) = <Self as PolynomialCommitment<G>>::setup(max_degree).unwrap();
    ...
    pub fn open_reduction_polynomials<'b>(
        ck: &CommitterKey<G>,
        xi_s: impl IntoIterator<Item = &'b SuccinctCheckPolynomial<G::ScalarField>>,
        point: G::ScalarField,
...
impl<G: Curve, D: Digest> PolynomialCommitment<G> for InnerProductArgPC<G, D> {
    ...
    fn setup(max_degree: usize) -> Result<(Self::CommitterKey, Self::VerifierKey), Self::Error> {
        Self::setup_from_seed(max_degree, &Self::PROTOCOL_NAME)
    }

    /// Setup of the base point vector (deterministically derived from the
    /// given byte array as seed).
    fn setup_from_seed(max_degree: usize, seed: &[u8]) -> Result<(Self::CommitterKey, Self::VerifierKey), Self::Error> {
