...
use algebra::{
    serialize::*, Field, Group, LinearCombination, SemanticallyValid,
};
use digest::Digest;
...
pub trait PolynomialCommitment<G: Group>: Sized {
    ...
    /// The commitment to a polynomial. Needs to be recordable.
    ...
    fn succinct_multi_point_multi_poly_verify<'a>(
        ...
        evaluations: &Evaluations<G::ScalarField>,
        multi_point_proof: &Self::MultiPointProof,
        // This implementation assumes that the commitments, query set and evaluations are already recorded by the Fiat Shamir rng
        fs_rng: &mut Self::RandomOracle,
    ) -> Result<Option<Self::VerifierState>, Self::Error> {
        let (combined_commitment, combined_value, combined_point) =
            Self::multi_point_multi_poly_verify_combine(
    ...
    > {
        ...
        // of the fr_rng, we simply get the challenge scalar for the random linear combination
    ...
    >
    {
        ...
        // we simply get the challenge scalar for the random linear combination
        let lambda = Self::challenge_to_scalar(
            fs_rng
                .get_challenge::<128>()?
                .to_vec()
