...
use algebra::{
    serialize::*, to_bytes, Field, Group, LinearCombination, SemanticallyValid, ToBytes,
};
...
pub type QueryMap<F> = BTreeMap<PointLabel, (F, BTreeSet<PolynomialLabel>)>;
...
pub trait PolynomialCommitment<G: Group>: Sized {
    ...
    /// The function assumes that the pairs `(polynomial, randomness)` produced by
    /// iterating simultaneously through `labeled_polynomials` and `labeled_randomnesses`
    /// are consistent, i.e. `randomness` is the hiding randomness to be applied to polynomial
    /// `polynomial`.
    /// Note: this default implementation demands that the state of the Fiat-Shamir
    ...
    /// The function assumes that the pairs `(commitment, value)` produced by
    /// iterating simultaneously through `labeled_commitments` and `values` are
    /// consistent, i.e. `value` is the evaluation of a given polynomial queried
    /// at `point` whose commitment is `commitment`.
    fn succinct_single_point_multi_poly_verify<'a>(
        ...
        let combined_value = LinearCombination::new_from_val(
            &lambda,
            values
                .into_iter()
                .collect()
    ...
    fn succinct_multi_point_multi_poly_verify<'a>(
        ...
        // This implementation assumes that the commitments, query map and evaluations are already absorbed by the Fiat Shamir rng
        fs_rng: &mut Self::RandomOracle,
    ) -> Result<Option<Self::VerifierState>, Self::Error> {
