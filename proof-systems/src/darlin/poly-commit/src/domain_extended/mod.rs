...
use crate::{PCProof, PCCommitterKey};
use algebra::{
    ...
    G::ScalarField: SquareRootField // Temporary
{
    ...
    ) -> Result<Self::Proof, Self::Error>
    {
        ...
        if key_len.next_power_of_two() != key_len {
            Err(Error::Other("Commiter key length is not power of 2".to_owned()))?
        }
        ...
        for i in (0..segments_count).into_iter() {
            ...
            if is_hiding {
                ...
                //  randomness segments
                randomnesses_lc.push(coeff.clone(), if randomness.len() <= i { PC::Randomness::zero() } else { randomness[i].clone() });
            }
        }
    ...
    ) -> Result<Option<Self::VerifierState>, Self::Error>
    {
        ...
        let commitments_lc = LinearCombination::<PC::Commitment>::new(combined_commitment.iter().enumerate().map(|(i, item)| {
            if i > 0 {
                coeff = coeff * &point_pow;
            }
