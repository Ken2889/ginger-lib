...
pub trait EvaluationDomain<F: PrimeField>: Debug + Send + Sync
{
    ...
    // TODO: let us change this to the following more elegant reindexing.
    // Let `q = |G|/|S|` the number of coset of the subgroup `S` in `G`. Then for any 
    // index `i` of an element in `G`, consider its quotient remainder decomposition 
    //      `i = i_2 * q + i_1`,
    // with `i_1 = i mod q` and `i_2 = (i - i_1)/q`. Then `i_1` corresponds to the coset number
    // from `{0,...,q-1}`, and `i_2` to the relative index in this coset (reaching from `0` up to 
    // `|S|-1`). The reindexed index is simply
    //      `reind(i) = i_1 * q + i_2 `.
