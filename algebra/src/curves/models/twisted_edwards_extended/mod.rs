...
impl<P: Parameters> CanonicalSerialize for AffineRep<P> {
    ...
    fn uncompressed_size(&self) -> usize {
        self.x.serialized_size() + self.y.serialized_size_with_flags::<EdwardsFlags>()
    }
}
...
impl<P: Parameters> CanonicalDeserialize for AffineRep<P> {
    ...
    fn deserialize<R: Read>(reader: R) -> Result<Self, SerializationError> {
        ...
        if !TEExtended::<P>::from_affine(&p).is_in_correct_subgroup_assuming_on_curve() {
    ...
    fn deserialize_uncompressed<R: Read>(reader: R) -> Result<Self, SerializationError> {
        ...
        if !TEExtended::<P>::from_affine(&p).group_membership_test() {
            return Err(SerializationError::InvalidData);
        }
        Ok(p)
    }
...
// The affine point X, Y is represented in the TEExtended
