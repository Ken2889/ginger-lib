use crate::{
    ...
    BigInteger320, FromBytes, SemanticallyValid,
};
use hex_literal::hex;
...
fn to_internal_repr(mut x: Vec<u8>, mut y: Vec<u8>) -> Secp256k1Jacobian {
    ...
    x.reverse();
    x.resize_with(40, || 0);
    let x = Fq::from_repr(BigInteger320::read(&x[..]).unwrap());

    y.reverse();
    y.resize_with(40, || 0);
    let y = Fq::from_repr(BigInteger320::read(&y[..]).unwrap());

    Secp256k1Jacobian::from_affine(&<Secp256k1Jacobian as Curve>::AffineRep::new(x, y))
}
...
fn test_secp256k1_addition_correctness() {
    ...
    for (i, (x, y)) in ADD_TEST_VECTORS.iter().enumerate() {
        ...
        curr_point = (curr_point + &gen).normalize();
    }
}
